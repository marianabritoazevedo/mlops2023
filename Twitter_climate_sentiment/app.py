...
def make_prediction(preprocessed_text: pd.Series,
    ...
    """
    tokenized = tokenizer(
        list(preprocessed_text), truncation=True, padding=True
    )
    outputs = model.predict(tokenized)
    classification = np.argmax(outputs, axis=1)

    labels = {0: 'Anti', 1: 'Neutral', 2: 'Pro', 3: 'News'}
    return labels[classification[0]]
...
def classify_tweet(tweet: str, model, tokenizer) -> str:
    ...
    return make_prediction(preprocessed, model, tokenizer)
...
if __name__ == "__main__":
    ...
    gr_interface = gr.Interface(
        title="Twitter Climate Change Sentiment",
        description=(
            "Classify your feelings about climate change "
            "based on a tweet. Classifications: \n"
            "- `3(News)`: Factual news; \n"
            "- `2(Pro)`: Supports man-made climate change belief; \n"
            "- `1(Neutral)`: Neutral; \n"
            "- `0(Anti)`: Does not believe in it.\n"
        ),
        article=(
