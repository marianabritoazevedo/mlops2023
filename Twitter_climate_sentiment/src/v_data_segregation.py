...
def data_segregation() -> None:
    ...
    with mlflow.start_run(run_name="data_segregation"):
        ...
        logger.info(f"🚂 Train set size: Features - {len(train_x)}, Labels - {len(train_y)}")
        logger.info(f"✅ Validation set size: Features - {len(val_x)}, Labels - {len(val_y)}")
        logger.info(f"🧪 Test set size: Features - {len(test_x)}, Labels - {len(test_y)}")
