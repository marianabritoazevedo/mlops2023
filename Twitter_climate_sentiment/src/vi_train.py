...
def download_segregated_data(logger, log_file, file_handler) -> str:
    """
    ...
    if run_id:
        ...
        for file_name in ['train_x.joblib', 'train_y.joblib', 'val_x.joblib', 'val_y.joblib', 'test_x.joblib', 'test_y.joblib']:
            client.download_artifacts(run_id, file_name, data_dir)
        logger.info(f"✅ Data from {run_name} downloaded successfully.")
        return data_dir
    else:
        raise Exception(f"{run_name} data not found in MLflow.")


def training(logger, log_file, file_handler) -> None:
    """
    Train the model.
    ...
    data_dir = download_segregated_data(logger, log_file, file_handler)
    ...
    train_encodings = tokenizer(list(train_x), truncation=True, padding='max_length', return_tensors='tf')
    val_encodings = tokenizer(list(val_x), truncation=True, padding='max_length', return_tensors='tf')
    logger.info("✨ Data tokenization completed.")

    # Create TensorFlow datasets
    train_dataset = tf.data.Dataset.from_tensor_slices((dict(train_encodings), tf.constant(train_y.values, dtype=tf.int32)))
    val_dataset = tf.data.Dataset.from_tensor_slices((dict(val_encodings), tf.constant(val_y.values, dtype=tf.int32)))
    ...
    logger.info(f"✅ TensorFlow model successfully logged to MLflow.")
...
def train() -> None:
    ...
    with mlflow.start_run(run_name="train_model"):
        training(logger, log_file, file_handler)
